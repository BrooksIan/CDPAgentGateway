...
| ID | Case | Expected | Automated |
| --- | --- | --- | --- |
| AMP-01 | AMP metadata present; `launchable` false | `.project-metadata.yaml` tasks; install is `run_session`; no `KNOX_TOKEN` project env; mcp-spark bypasses CML login; admin does not; apps poll `/health` | `tests/test_amp_packaging.py` |
