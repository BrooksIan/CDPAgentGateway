...
4. LangGraph notebook: Knox JWT, **model form** (URL / id / token), bind MCP tools, then a read-only ReAct turn. Custom model URLs omit vLLM-incompatible `tool_choice=auto`. AMP pip warnings about CML `protobuf` / `typing-extensions` pins can be ignored; the notebook no longer changes protobuf. Set `LANGGRAPH_RUN_SUBMIT=1` only if the job file is already staged.
