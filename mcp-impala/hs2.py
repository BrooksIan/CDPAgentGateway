...
try:
...
except ImportError:  # Compose mcp-impala image has no agentgateway package
    import inspect
    ...
    def _impyla_connect(connect, kwargs: dict[str, Any]):
        ...
        if kwargs.get("jwt") and "jwt" not in filtered:
            raise ImpalaError("Impala client is too old for JWT; pip install 'impyla>=0.19'", status=500)
