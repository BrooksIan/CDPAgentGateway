#!/usr/bin/env python3
"""Impala MCP adapter. Read-only list/describe/select as the Knox token subject."""
...
from sql import ImpalaError

PROTOCOL_VERSION = "2025-03-26"
SERVER_NAME = "mcp-impala"
SERVER_VERSION = "0.1.0"
PORT = int(os.environ.get("PORT", "8080"))

TOOLS = [
    {
        "name": "impala_list_databases",
        "description": (
            "List Impala databases Ranger allows for the Knox token subject. "
            "Read-only. /cdp/impala stays unpublished."
        ),
        "inputSchema": {"type": "object", "properties": {}, "additionalProperties": False},
    },
    {
        "name": "impala_list_tables",
        "description": "List tables in one Impala database for the Knox token subject. Read-only.",
        "inputSchema": {
            "type": "object",
    ...
    {
        "name": "impala_describe_table",
        "description": "Describe columns on one Impala table. Read-only. No DDL.",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        "name": "impala_select",
        "description": (
            "SELECT named columns from one Impala table as the Knox token subject. "
    ...
    if method == "initialize":
        return _rpc_result(
            rpc_id,
            {
                ...
                "instructions": (
                    "Read-only Impala tools over Knox Impala HS2. "
                    "Send the caller's Knox JWT as Authorization. "
                    "Ranger authorizes that subject. "
                    "Do not call /cdp/impala or raw Knox. No DDL/DML. No SELECT *."
                ),
            },
        )
    if method == "ping":
...
def _call_tool(rpc_id: Any, name: str, arguments: dict[str, Any], request: Request) -> JSONResponse:
    ...
    try:
        ...
        if name == "impala_list_databases":
            result = list_databases(
                authorization=authorization,
                knox_user=knox_user,
                request_id=request_id,
            )
        elif name == "impala_list_tables":
            result = list_tables(
        ...
        return _tool_error(rpc_id, "impala request failed", {"status": 502})
