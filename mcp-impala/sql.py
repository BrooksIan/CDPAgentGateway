"""Build read-only Impala SQL from structured tool arguments. No free-form statements."""
...
def ident(raw: Any, *, field: str) -> str:
    ...
    if not IDENT.match(name):
        raise ImpalaError(f"{field} must be an Impala identifier", status=400)
    return name
...
def parse_limit(raw: Any, *, default: int = 20) -> int:
    if raw is None or raw == "":
        ...
        raise ImpalaError("limit must be an integer", status=400)
    ...
    if value < 1 or value > MAX_ROWS:
        raise ImpalaError(f"limit must be between 1 and {MAX_ROWS}", status=400)
    return value
...
def column_list(raw: Any) -> list[str]:
    if raw is None:
        ...
        raise ImpalaError("columns must be a list of identifiers", status=400)
    if not items:
        raise ImpalaError("columns are required; SELECT * is not allowed", status=400)
    if len(items) > MAX_COLS:
        raise ImpalaError(f"columns are limited to {MAX_COLS}", status=400)
